...
export const fetchRoleStart = () => {
  return async (dispatch, getState) => {
    try {
      ...
      dispatch(fetchRoleFailed());
      console.log("fetchRolesStart error", e);
    }
  };
};
...
export const editAUser = (data) => {
  return async (dispatch, getState) => {
    try {
      ...
      console.log("TAi chekc: ", res);
      if (res && res.errCode === 0) {
        toast.success("Update the user succed!");
        dispatch(editUserSuccess());
        dispatch(fetchAllUsersStart());
      } else {
        toast.error("Update alsc users error!");
        dispatch(editUserFailed());
      }
    } catch (e) {
      toast.error("Update allsc users error");
      dispatch(editUserFailed());
...
export const saveDetailDoctor = (data) => {
  return async (dispatch, getState) => {
    try {
      ...
      if (res && res.errCode === 0) {
        toast.success("Save infor detail doctors succeed!");
        dispatch({
          type: actionTypes.SAVE_DETAIL_DOCTOR_SUCCESS,
        });
      } else {
        console.log("err res: ", res);
        toast.error("Save infor detail doctors error!");
